...
export const AI_MODELS: AIModel[] = [
  ...
  },
  {
    ...
    description: "Deep research AI by (Claude-opus-4) enhanced by Cognix",
    badge: "Deep Research",
  },
  {
...
export async function editImage(
  ...
  try {
    console.log(
      "Editing image with black-forest-labs-flux-1-kontext-max model, prompt:",
      prompt
    );
...
export async function* generateAIResponseStream(
  ...
  try {
    ...
    if (lastUserMessage?.role === "user") {
      ...
      if (shouldShowVideos(lastUserMessage.content)) {
        try {
          ...
          const searchQuery = extractVideoSearchKeywords(
      ...
      if (shouldSearch) {
        try {
          ...
          if (searchResults.length > 0) {
            ...
            yield {
              content:
                "No relevant search results found. Providing general information...\n\n",
              videos: youtubeVideos,
            };
          }
        } catch (error) {
          ...
          yield {
            content:
              "Web search temporarily unavailable. Providing general information...\n\n",
            videos: youtubeVideos,
          };
        }
      }
    ...
    const nameFromMessages = enhancedMessages
      .find(
        (msg) =>
          msg.role === "user" && /my name is ([a-zA-Z]+)/i.test(msg.content)
      )
...
${
  userName
    ? `USER CONTEXT: The user's name is ${userName}. Remember this information throughout the conversation.`
    : ""
}

${
  isWebSearchTriggered
    ? "🌐 **Web search was performed** - Use the provided search results to give accurate, current information. Present it naturally as if you knew it directly."
    : ""
}

${
  youtubeVideos.length > 0
    ? `📹 **YouTube videos found** - Relevant educational videos have been provided above your response. Reference these videos when appropriate in your answer.`
    : ""
}
